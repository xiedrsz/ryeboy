      ...
      <swipe ref="swipe" @slidechanged="slideChanged">
        <swipe-slide v-for="channel in channels" :id="channel.id">
          <div class="slide-content">
            ...
            <loadable-content :nomore="dNomore[channel.id]" loadstate="infin" :infinite="getMore(channel.id)">
              <ul class="mdl-list">
                <diary-item v-for="item in diaries[channel.id]" :key="item._id" :id="item._id" :userid="item.userid" :like="item.like" :likeCount="item.likeCount" :commentCount="item.commentCount" :avatar="item.avatar" :username="item.username" :pictures="item.pictures"
                :verified="item.verified" :text="item.escapedText" :userlv="item.userlv" :expectedExp="item.expectedExp" :time="item.time">
                </diary-item>
              </ul>
            </loadable-content>
...
//      !diaries[0] && this.getDiary();
    ...
    computed: {
      channels() {
        ...
        dNomore() {
          return this.$store.state.group.dNomore;
        },
    },
    methods: {
      // 侧滑
      slideChanged(index) {
        ...
        getMore(filter) {
          ...
          async function infinite() {
            await that.$store.dispatch("getGroupDiaries", {
              filter
            });
          }
          return infinite;
